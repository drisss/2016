...
function toString(obj, params, ref)
  ...
  if type(obj) == "table" then
    ...
    local t = obj
    local repr = "{ref."..getmetatable(ref).counter..(params.ref and (" "..tostring(obj)) or "").."\n"
    local first = true
    ...
    for _, k in true_ipairs(keys) do
      ...
      local r = ref[k]
      if not r then
        repr = repr..toString(k, {depth = params.depth, level = params.level + 1, meta = params.meta, ref = params.ref}, ref)
      else
        repr = repr.."[ref_to."..r.."]"
      end
      ...
      if not r then
        if type(v) == "string" then
          ...
          repr = repr..toString(v, {depth = params.depth, level = params.level + 1, meta = params.meta, ref = params.ref}, ref)
        end
      else
        repr = repr.."[ref_to."..r.."]"
      end
    ...
    if meta and params.meta then
      ...
      repr = repr.."metatable = "..toString(meta, {depth = params.depth, level = params.level + 1, meta = params.meta, ref = params.ref}, ref)
    end
